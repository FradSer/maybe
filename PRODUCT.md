...
**Secondary user** (inferred from A2A protocol implementation): External AI agents that interact with the user's financial data via the Agent-to-Agent protocol (JSON-RPC 2.0 over HTTP).
...
- **Agent integration**: A2A protocol endpoint at `/api/v1/a2a` for programmatic agent access
...
- AI assistant/chat for natural-language financial queries
- A2A Agent-to-Agent protocol for external AI agent integration
- Demo mode for evaluation without real data
- Dark/light theme support
- User roles: personal (single-user or family accounts)
...
- Rails 7.2 monolith with Hotwire frontend (no separate SPA)
- PostgreSQL database
...
- Ruby on Rails 7.2 application with Hotwire (Turbo + Stimulus) frontend
...
- No ARIA-specific audit evidence found in surface scan